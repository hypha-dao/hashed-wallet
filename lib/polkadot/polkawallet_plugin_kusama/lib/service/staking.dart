...
class ApiStaking {
  ...
  Future<Map?> fetchAccountRewards(int? eras) async {
    if (store!.staking.ownStashInfo != null && store!.staking.ownStashInfo!.stakingLedger != null) {
      int bonded = int.parse(store!.staking.ownStashInfo!.stakingLedger!['active'].toString());
  ...
  Future<Map> queryOwnStashInfo() async {
    final dynamic data = await api.service.staking.queryOwnStashInfo(keyring.current.address!);
    store!.staking.setOwnStashInfo(keyring.current.pubKey, data);

    final List<String?> addressesNeedIcons =
        store!.staking.ownStashInfo?.nominating != null ? store!.staking.ownStashInfo!.nominating!.toList() : [];
    ...
    if (iconsAndPubKeys[0] != null && iconsAndPubKeys[1] != null) {
      ...
      store!.accounts
          .setPubKeyAddressMap(Map<String, Map>.from({api.connectedNode!.ss58.toString(): iconsAndPubKeys[1]}));
    }
  ...
  Future<void> queryAccountBondedInfo() async {
    final List<String> accounts = keyring.allAccounts.map((e) => e.pubKey!).toList();
