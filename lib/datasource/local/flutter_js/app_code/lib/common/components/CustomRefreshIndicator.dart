...
class CustomRefreshIndicatorState extends State<CustomRefreshIndicator>
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    ...
    if (indicatorAtTopNow != _isIndicatorAtTop) {
      if (_mode == CustomRefreshIndicatorMode.drag || _mode == CustomRefreshIndicatorMode.armed)
      ...
      if (_mode == CustomRefreshIndicatorMode.armed && notification.dragDetails == null) {
  ...
  void _checkDragOffset(double containerExtent) {
    assert(_mode == CustomRefreshIndicatorMode.drag || _mode == CustomRefreshIndicatorMode.armed);
    double newValue = _dragOffset / (containerExtent * _kDragContainerExtentPercentage);
    if (_mode == CustomRefreshIndicatorMode.armed) newValue = math.max(newValue, 1.0 / _kDragSizeFactorLimit);
    _positionController.value = newValue.clamp(0.0, 1.0); // this triggers various rebuilds
    if (_mode == CustomRefreshIndicatorMode.drag && _valueColor.value.alpha == 0xFF)
  ...
  Future<void> dismiss(CustomRefreshIndicatorMode newMode) async {
    ...
    // direction change.
    assert(newMode == CustomRefreshIndicatorMode.canceled || newMode == CustomRefreshIndicatorMode.done);
    setState(() {
      _mode = newMode;
    });
    switch (_mode) {
  ...
  Future<void> show({bool atTop = true}) {
    if (_mode != CustomRefreshIndicatorMode.refresh && _mode != CustomRefreshIndicatorMode.snap) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool showIndeterminateIndicator =
        _mode == CustomRefreshIndicatorMode.refresh || _mode == CustomRefreshIndicatorMode.done;
