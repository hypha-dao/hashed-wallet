...
abstract class _SettingsStore with Store {
  ...
  @observable
  Map<String, List<MessageData>> communityMessages = Map<String, List<MessageData>>();
  ...
  Future<void> initMessage(String _languageCode) async {
    ...
    if (dataCommunity != null && dataCommunity.length > 0) {
      final all = List.of(dataCommunity).map((element) => MessageData.fromJson(element)).toList();
      final Map<String, List<MessageData>> map = Map<String, List<MessageData>>();
      all.forEach((element) {
    ...
    if (dataSystem != null && dataSystem.length > 0) {
      final all = List.of(dataSystem).map((element) => MessageData.fromJson(element)).toList();
      setSystemMessages(all);
    }
    if (stored != null) {
      final Map<String, String> storedMap = new Map<String, String>.from(json.decode(stored));
      final Map<String, int> map = Map<String, int>();
      communityMessages.forEach((key, value) {
        map[key] = value.where((element) => storedMap[element.file] == null).toList().length;
      });
      setCommunityUnreadNumber(map);

      final int system = systemMessages.where((element) => storedMap[element.file] == null).toList().length;
  ...
  Future<void> readSystmeMessage(List<MessageData> datas, String network) async {
    ...
    final system = systemMessages.where((element) => storedMap[element.file] == null).toList().length;
    setSystemUnreadNumber(system);
  }

  Future<void> readCommunityMessage(List<MessageData> datas, String network) async {
    final storedMap = getReadMessage();
    var isNew = false;
    datas.forEach((element) {
      if (storedMap[element.file] == null) {
    ...
    map[network] =
        (communityMessages[network] ?? []).where((element) => storedMap[element.file] == null).toList().length;
    map['all'] = (communityMessages['all'] ?? []).where((element) => storedMap[element.file] == null).toList().length;
    setCommunityUnreadNumber(map);
  }
