...
class ApiAccount {
  ...
  Future<Map> importAccount({
    ...
    if (isFromCreatePage && (acc.name == null || acc.name.isEmpty || acc.password == null || acc.password.isEmpty)) {
  ...
  void setBiometricEnabled(String pubKey) {
    apiRoot.store.storage.write('$_biometricEnabledKey$pubKey', DateTime.now().millisecondsSinceEpoch);
  }
  ...
  bool isCloseBiometricDisabled(String pubKey) {
    final timestamp = apiRoot.store.storage.read('$_biometricEnabledKey$pubKey');
  ...
  bool getBiometricEnabled(String pubKey) {
    ...
    if (timestamp != null && timestamp + SECONDS_OF_DAY * 7000 > DateTime.now().millisecondsSinceEpoch) {
      return true;
    }
    return false;
  }
  ...
  // Future<String> getPasswordWithBiometricAuth(BuildContext context, String pubKey) async {
      ...
      await showCupertinoDialog(
        ...
        builder: (BuildContext context) {
          return CupertinoAlertDialog(
            title: Text(I18n.of(context).getDic(i18n_full_dic_app, 'assets')['note']),
            content: Text(I18n.of(context).getDic(i18n_full_dic_app, 'account')['biometric.msg']),
            actions: <Widget>[
              CupertinoButton(
    ...
    final password = await showCupertinoDialog(
      ...
      builder: (_) {
        return PasswordInputDialog(
          ...
          title: Text(I18n.of(context).getDic(i18n_full_dic_app, 'account')['unlock']),
          account: acc,
  ...
  Future<void> queryAddressIcons(List addresses) async {
    addresses.retainWhere((e) => !apiRoot.store.account.addressIconsMap.containsKey(e));
    if (addresses.length == 0) return;

    final icons = await apiRoot.plugin.sdk.api.account.getAddressIcons(addresses);
