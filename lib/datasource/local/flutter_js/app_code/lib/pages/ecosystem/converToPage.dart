...
import 'package:seeds/datasource/local/flutter_js/app_code/lib/pages/ecosystem/transitingWidget.dart';
...
class _ConverToPageState extends State<ConverToPage> {
  ...
  Future<XcmTxConfirmParams> _getTxParams() async {
    if (_error1 == null && _amountCtrl.text.trim().length > 0) {
      ...
      if (xcmParams != null) {
        ...
        final feeToken = plugin.store.assets.allTokens
        ...
        return XcmTxConfirmParams(
            ...
            call: xcmParams['call'],
            txDisplay: {
              dicAcala['cross.chain']: widget.service.plugin.basic.name?.toUpperCase(),
            },
            txDisplayBold: {
              dic['amount']: Text(
            ...
            feeToken: feeToken,
            isPlugin: true,
            waitingWidget: TransitingWidget(fromNetwork, widget.service.plugin.basic.name, balance.symbol));
      }
    }
    return null;
  }
  ...
  Future<Map> _getXcmParams(String amount) async {
    ...
    final chainToInfo = (tokensConfig['xcmChains'] ?? {})[widget.service.plugin.basic.name] ?? {};
    final sendFee = List.of((tokensConfig['xcmSendFee'] ?? {})[widget.service.plugin.basic.name] ?? []);
    ...
    if (mounted) {
      setState(() {
        if (_amountCtrl.text.trim().length > 0) {
          _receiver = (Fmt.tokenInt(_amountCtrl.text.trim(), balance.decimals) - Fmt.balanceInt(_fee)).toString();
        }
        _isLoading = false;
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    final destExistDeposit = Fmt.balanceInt(plugin.store.assets.tokenBalanceMap[balance.tokenNameId].minBalance);
    final destFee = Fmt.balanceInt((tokenXcmInfo[balance.symbol] ?? {})['receiveFee']);

    final nativeToken = widget.service.plugin.networkState.tokenSymbol[0];
    final nativeTokenDecimals = widget
        .service.plugin.networkState.tokenDecimals[widget.service.plugin.networkState.tokenSymbol.indexOf(nativeToken)];
              ...
              Expanded(
                ...
                children: [
                  ...
                  PluginInputBalance(
                    ...
                    titleTag: "${dic['ecosystem.bringTo']} ${widget.service.plugin.basic.name}",
              ...
              Padding(
                  ...
                  child: PluginButton(
                    ...
                    onPressed: () async {
                      ...
                      if (params != null) {
                        final res = await Navigator.of(context).pushNamed(XcmTxConfirmPage.route, arguments: params);
                        if (res != null) {
...
class ErrorMessage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      margin: error == null ? EdgeInsets.zero : margin ?? EdgeInsets.only(left: 16, top: 4),
      child: error == null
          ? null
          : Row(children: [
              Expanded(
                  child: Text(
                error,
                style: TextStyle(fontSize: UI.getTextSize(12, context), color: Colors.red),
              ))
            ]),
    );
  }
