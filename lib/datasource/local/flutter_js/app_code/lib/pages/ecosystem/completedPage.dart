...
class _CompletedPageState extends State<CompletedPage> {
  ...
  _getBalance(List<String> networkNames) async {
    ...
    Map<String, TokenBalanceData> balances =
        await TokenStakingApi.getBalance(widget.service, networkNames, balance.symbol, isCachaChange: false);
  ...
  Widget build(BuildContext context) {
    ...
    final style = Theme.of(context).textTheme.headline4?.copyWith(color: PluginColorsDark.headline1, height: 2.0);
    ...
    return PluginScaffold(
        ...
        body: Observer(builder: (_) {
          final currentBalance =
              widget.service.plugin.noneNativeTokensAll.firstWhere((element) => element.symbol == balance.symbol);
          return SafeArea(
                          ...
                          Container(
                            ...
                            child: Column(
                              children: [
                                ...
                                InfoItemRow(
                                  I18n.of(context).getDic(i18n_full_dic_karura, 'acala')['transfer.fee'],
                                    ...
                                    children: [
                                      Expanded(
                                        child: PluginButton(
                                          ...
                                          onPressed: () => Navigator.of(context)
                                              .popUntil((route) => route.settings.name == TokenStaking.route),
                                        ),
                                      ),
                                      Container(
                                        width: 16,
                                      ...
                                      Expanded(
                                        child: PluginButton(
                                          ...
                                          title: "${dic['ecosystem.convertTo']} $convertToKen",
                                          onPressed: () async {
                                            final convertBalance = widget.service.plugin.noneNativeTokensAll
                                                .firstWhere((element) => element.symbol == convertToKen);
                                            if (convertToKen.startsWith("L")) {
                                              //to mint
                                              final res = await Navigator.of(context).pushNamed(
                                                  "/${widget.service.plugin.basic.name.toLowerCase()}/homa/mint");
                                              if (res != null) {
                                                convertBalance.amount =
                                                    Fmt.tokenInt(res, convertBalance.decimals).toString();
