...
class _CrossChainTransferPageState extends State<CrossChainTransferPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final tokenXcmConfig = List<String>.from((tokensConfig['xcm'] ?? {})[balance.tokenNameId] ?? []);
  ...
  Future<XcmTxConfirmParams> _getTxParams() async {
    if (_error1 == null && _amountCtrl.text.trim().length > 0) {
      ...
      final xcmParams = await _getXcmParams(Fmt.tokenInt(_amountCtrl.text.trim(), balance.decimals).toString());
      var plugin;
      if (widget.service.plugin is PluginKarura) {
        plugin = widget.service.plugin as PluginKarura;
      } else if (widget.service.plugin is PluginAcala) {
      ...
      final fromIcon = plugin.store.assets.crossChainIcons[fromNetwork] as String;
      if (xcmParams != null) {
        var plugin;
        if (widget.service.plugin is PluginKarura) {
          plugin = widget.service.plugin as PluginKarura;
        ...
        final tokensConfig = plugin.store.setting.remoteConfig['tokens'] ?? {};
        final feeTokenSymbol = ((tokensConfig['xcmChains'] ?? {})[fromNetwork] ?? {})['nativeToken'];
        final feeToken = (fromNetwork == para_chain_name_acala || fromNetwork == para_chain_name_karura)
            ? TokenBalanceData(
                symbol: plugin.networkState.tokenSymbol[0],
                decimals: plugin.networkState.tokenDecimals[0],
              )
            : plugin.store.assets.allTokens
                .firstWhere((e) => e.symbol.toUpperCase() == feeTokenSymbol.toString().toUpperCase());
        return XcmTxConfirmParams(
            txTitle: '${dicAcala['transfer']} ${balance.symbol} (${dicAcala['cross.xcm']})',
            ...
            txDisplayBold: {
              dic['amount']: Text(
                Fmt.priceFloor(double.tryParse(_amountCtrl.text.trim()), lengthMax: 8) + ' ${balance.symbol}',
                style: Theme.of(context).textTheme.headline1?.copyWith(color: PluginColorsDark.headline1),
              ),
              dic['address']: Row(
                children: [
                  AddressIcon(widget.service.keyring.current.address, svg: widget.service.keyring.current.icon),
                  Expanded(
                    child: Container(
                      margin: EdgeInsets.fromLTRB(8, 16, 0, 16),
                      child: Text(
                        Fmt.address(widget.service.keyring.current.address, pad: 8),
                        style: Theme.of(context).textTheme.headline4?.copyWith(color: PluginColorsDark.headline1),
                      ),
                    ),
                  ),
                ],
            ...
            chainFromIcon: fromIcon.contains('.svg') ? SvgPicture.network(fromIcon) : Image.network(fromIcon),
            feeToken: feeToken,
            isPlugin: true);
      }
    }
  ...
  Future<Map> _getXcmParams(String amount, {bool feeEstimate = false}) async {
    ...
    final Map xcmParams = await widget.service.plugin.sdk.webView?.evalJavascript('xcm.getTransferParams('
  ...
  Future<String> _getTxFee({reload = false}) async {
    ...
    final sender = TxSenderData(widget.service.keyring.current.address, widget.service.keyring.current.pubKey);
    ...
    if (fromNetwork == plugin_name_karura || fromNetwork == plugin_name_acala) {
      final feeData = await widget.service.plugin.sdk.api.tx.estimateFees(txInfo, xcmParams['params']);
  ...
  Future<void> _selectChain(
      BuildContext context, int index, Map<String, Widget> crossChainIcons, List<String> options) async {
  ...
  Widget build(BuildContext context) {
    ...
    final labelStyle = Theme.of(context).textTheme.headline4?.copyWith(color: PluginColorsDark.headline1);
    final subTitleStyle = TextStyle(fontSize: UI.getTextSize(12, context), height: 1);
    final infoValueStyle =
        Theme.of(context).textTheme.headline5.copyWith(fontWeight: FontWeight.w600, color: PluginColorsDark.headline1);
    return PluginScaffold(
        ...
        body: Observer(builder: (_) {
          ...
          final tokensConfig = plugin.store.setting.remoteConfig['tokens'] ?? {};
          final crossChainIcons = Map<String, Widget>.from(plugin.store.assets.crossChainIcons.map((k, v) =>
              MapEntry(k.toUpperCase(), (v as String).contains('.svg') ? SvgPicture.network(v) : Image.network(v))));

          final destChainName = _chainTo;

          final isFromKar = fromNetwork == plugin_name_karura;

          final tokenXcmInfo = (tokensConfig['xcmInfo'] ?? {})[isFromKar ? destChainName : fromNetwork] ?? {};

          final isTokenFromStateMine = balance.src != null && balance.src['Parachain'] == '1,000';

          final existDeposit = Fmt.balanceInt(plugin.store.assets.tokenBalanceMap[balance.tokenNameId].minBalance);
          final destExistDeposit =
              isFromKar ? Fmt.balanceInt((tokenXcmInfo[balance.symbol] ?? {})['existentialDeposit']) : existDeposit;
          ...
          final sendFee = List.of((tokenXcmInfo[balance.symbol] ?? {})['sendFee'] ?? []);

          final sendFeeAmount = sendFee.length > 0 ? Fmt.balanceInt(sendFee[1]) : BigInt.zero;

          final feeTokenSymbol = ((tokensConfig['xcmChains'] ?? {})[fromNetwork] ?? {})['nativeToken'];

          final nativeToken = widget.service.plugin.networkState.tokenSymbol[0];
          final nativeTokenDecimals = widget.service.plugin.networkState
              .tokenDecimals[widget.service.plugin.networkState.tokenSymbol.indexOf(nativeToken)];
          return SafeArea(
              child: Padding(
                          ...
                          isFromKar
                              ? PluginTagCard(
                                  ...
                                  child: GestureDetector(
                                    onTap: () {
                                      _selectChain(context, 0, crossChainIcons, _chainToList);
                                    },
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        ...
                                        Icon(Icons.keyboard_arrow_down_rounded, color: PluginColorsDark.headline1)
                                      ],
                                    ),
                                  ),
                                )
                              : Container(
                                  margin: EdgeInsets.only(top: 24),
                                  child: ChainSelected(dic['ecosystem.to'], destChainName),
                                ),
                          PluginInputBalance(
                            margin: EdgeInsets.only(top: 24, bottom: _error1 == null ? 24 : 2),
                            titleTag:
                                "${balance.symbol} ${I18n.of(context)?.getDic(i18n_full_dic_app, 'assets')['amount']}",
                            inputCtrl: _amountCtrl,
                            onInputChange: (v) {
                              var error = _validateAmount(v, Fmt.balanceInt(balance.amount), balance.decimals);
                              setState(() {
                                _error1 = error;
                              });
                            },
                            ...
                            children: [
                              Expanded(
                                child: Container(
                                    ...
                                    child: Text(dicAcala['cross.exist'], style: labelStyle)),
                              ),
                              Expanded(
                                  flex: 0,
                                  child: Text(
                              ...
                              children: [
                                Expanded(
                                  child: Padding(
                                    ...
                                    child: Text(dicAcala['cross.fee'], style: labelStyle),
                                  ),
                                ),
                                Text(
                                  ...
                                  Expanded(
                                    child: Container(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            Text(dicAcala['transfer.exist'], style: labelStyle),
                                            Text(dicAcala['cross.exist.msg'], style: subTitleStyle),
                                          ],
                                        )),
                                  ),
                                  Text(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Expanded(
                                      child: Padding(
                                        padding: EdgeInsets.only(right: 4),
                                        child: Text(dicAcala['transfer.fee'], style: labelStyle),
                                      ),
                                    ),
                                    Text(
                      ...
                      Padding(
                          ...
                          child: PluginButton(
                            ...
                            onPressed: () async {
                              ...
                              if (params != null) {
                                final res =
                                    await Navigator.of(context).pushNamed(XcmTxConfirmPage.route, arguments: params);
                                if (res != null) {
                                  Navigator.of(context).popAndPushNamed(EcosystemPage.route, arguments: {
                                    "balance": balance,
                                    "transferBalance": _amountCtrl.text.trim(),
                                    "convertNetwork": _chainTo ?? widget.service.plugin.basic.name,
                                    "type": "transferred"
                                  });
                                }
                              }
...
class ChainSelector extends StatelessWidget {
  ChainSelector({@required this.options, @required this.crossChainIcons, @required this.onSelect});
        ...
        Container(
            ...
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.only(
                    bottomLeft: Radius.circular(4), topRight: Radius.circular(4), bottomRight: Radius.circular(4))),
            child: Text(
              fromNetwork,
              style: Theme.of(context).textTheme.headline5?.copyWith(color: Color(0xFFFFFFFF).withAlpha(102)),
            )),
      ],
    );
  }
