...
class ImportAccountAction {
  static Future<void> authBiometric(BuildContext context, AppService service) async {
  ...
  static Future<bool> onSubmit(BuildContext context, AppService service, Map<String, dynamic> data,
    ...
    final _keyType = KeyType.values.firstWhere((e) => e.toString().contains(data['keyType']));
    ...
    try {
      ...
      if (acc == null) {
        ...
        showCupertinoDialog(
          ...
          builder: (BuildContext context) {
            return CupertinoAlertDialog(
              title: Container(),
              content: Text('${dic['import.invalid']} ${dic['create.password']}'),
              actions: <Widget>[
                CupertinoButton(
      ...
      final duplicated = await _checkAccountDuplicate(context, service, acc['pubKey']);
      ...
      if (err.toString().contains('Invalid')) {
        await showCupertinoDialog(
          ...
          builder: (BuildContext context) {
            return CupertinoAlertDialog(
              title: Container(),
              content: Text('${dic['import.invalid']} ${dic[_keyType.toString().split('.')[1]]}'),
              actions: <Widget>[
                CupertinoButton(
  ...
  static Future<bool> _checkAccountDuplicate(BuildContext context, AppService service, String pubKey) async {
    final dic = I18n.of(context).getDic(i18n_full_dic_app, 'account');
    final dicCommon = I18n.of(context).getDic(i18n_full_dic_ui, 'common');
    final index = service.keyring.keyPairs.indexWhere((i) => i.pubKey == pubKey);
    if (index > -1) {
