...
class _AccountAdvanceOption extends State<AccountAdvanceOption> {
  ...
  Future<String> _doCheckPath(String path) async {
    ...
    return widget.api.checkDerivePath(widget.seed, path, _typeOptions[_typeSelection]);
  }
                  ...
                  builder: (_) => Container(
                    ...
                    child: CupertinoPicker(
                      ...
                      children: _typeOptions
                          .map((i) => Padding(padding: EdgeInsets.all(16), child: Text(i.toString().split('.')[1])))
                          .toList(),
                      onSelectedItemChanged: (v) {
                        ...
                        if (_pathCtrl.text.isNotEmpty) {
                          error = _checkDerivePath(_pathCtrl.text, forceCheck: true);
                        }
                        widget.onChange(AccountAdvanceOptionParams(
                          type: _typeOptions[v],
                          // path: _derivePath,
...
class AccountAdvanceOptionParams {
  AccountAdvanceOptionParams({this.type = CryptoType.sr25519, this.path = '', this.error = false});
  CryptoType type;
  String path;
  bool error;
}
