...
class TxDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isKSMOrDOT =
        service.plugin.basic.name == relay_chain_name_ksm || service.plugin.basic.name == relay_chain_name_dot;
    final symbol =
        isKSMOrDOT ? service.plugin.networkState.tokenSymbol[0] : service.plugin.networkState.tokenSymbol ?? '';
    final decimals =
        isKSMOrDOT ? service.plugin.networkState.tokenDecimals[0] : service.plugin.networkState.tokenDecimals ?? 12;
    ...
    info.addAll(params.map((i) {
      ...
      switch (i['type']) {
        ...
        case "AccountId":
          ...
          String address = service.store.account.pubKeyAddressMap[service.plugin.sdk.api.connectedNode.ss58][value];
    ...
    return TxDetail(
      ...
      networkName: service.plugin.basic.isTestNet ? '${service.plugin.basic.name}-testnet' : service.plugin.basic.name,
