...
class _Settings extends State<SettingsPage> {
      ...
      builder: (_) => Container(
        ...
        child: WillPopScope(
          ...
          onWillPop: () async {
            ...
            if (code != cached) {
              widget.changeLang(code == '' ? Localizations.localeOf(context).toString() : code);
              setState(() {
                _isLoading = true;
              });
              Future.delayed(Duration(milliseconds: 1500), () {
                Navigator.of(context).pushNamedAndRemoveUntil(HomePage.route, (route) => false, arguments: {"tab": 0});
              });
            }
            return true;
          },
        ...
        child: WillPopScope(
          child: CupertinoPicker(
            ...
            children: _priceCurrencyOptions.map((i) {
              return Padding(padding: EdgeInsets.all(16), child: Text(_getPriceCurrency(i)));
            }).toList(),
            onSelectedItemChanged: (v) {
              selected = v;
            },
  ...
  Widget build(BuildContext context) {
    ...
    return _isLoading
        ...
        : Scaffold(
            appBar: AppBar(title: Text(dic['setting']), centerTitle: true, leading: BackBtn()),
            body: Observer(
              builder: (_) {
                final hideBalanceTip = widget.service.store.settings.isHideBalance ? dic['setting.currency.tip'] : '';
                final currencyTip =
                    widget.service.store.settings.priceCurrency == 'CNY' ? ' (${dic['setting.currency.tip']})' : '';
                return SafeArea(
                        ...
                        children: <Widget>[
                          SettingsPageListItem(
                            ...
                            content: v3.CupertinoSwitch(
                              ...
                              onChanged: (v) => widget.service.store.settings.setIsHideBalance(v),
                            ),
                          ),
                          Divider(height: 24.h),
                          SettingsPageListItem(
                            label: dic['setting.currency'],
                            subtitle: _getPriceCurrency(widget.service.store.settings.priceCurrency) + currencyTip,
                            onTap: _onCurrencyTap,
                          ),
                          Divider(height: 24.h),
                          SettingsPageListItem(
                            label: dic['setting.lang'],
                            subtitle: _getLang(widget.service.store.settings.localeCode),
