...
class _InitiateRecoveryPage extends State<InitiateRecoveryPage> {
  ...
  Future<void> _onValidateSubmit() async {
    ...
    final info = await widget.service.account.queryRecoverable(_recoverable.address);
    setState(() {
      _loading = false;
    });
    if (info == null) {
  ...
  Future<void> _onSubmit() async {
    final dic = I18n.of(context).getDic(i18n_full_dic_app, 'profile');
    final params =
        TxConfirmParams(txTitle: dic['recovery.init'], module: 'recovery', call: 'initiateRecovery', txDisplay: {
      'accountId': _recoverable.address,
      'deposit': '${Fmt.doubleFormat(_recoveryDeposit)} ${widget.service.plugin.networkState.tokenSymbol[0]}'
    }, params: [
            ...
            children: [
              ...
              Padding(
                ...
                child: RoundedButton(
                  ...
                  onPressed: _recoverable != null ? () => _onValidateSubmit() : null,
                  submitting: _loading,
                ),
              )
            ],
