...
class _Contact extends State<ContactPage> {
  ...
  Future<void> _onSave() async {
    ...
    if (_formKey.currentState.validate()) {
      ...
      if (_args == null) {
        // create new contact
        int exist = widget.service.keyring.contacts.indexWhere((i) => i.address == address);
        if (exist > -1) {
          showCupertinoDialog(
            context: context,
          ...
          final res = await widget.service.plugin.sdk.api.keyring.addContact(widget.service.keyring, con);

          if (_isObservation) {
            widget.service.plugin.changeAccount(res);
            widget.service.store.assets.loadCache(res, widget.service.plugin.basic.name);
          }
        }
      } else {
        // edit contact
        ...
        if (_args.pubKey == widget.service.keyring.store.currentPubKey && _args.observation && !_isObservation) {
          if (widget.service.keyring.allAccounts.length > 0) {
            widget.service.keyring.setCurrent(widget.service.keyring.allAccounts[0]);
            widget.service.plugin.changeAccount(widget.service.keyring.allAccounts[0]);
          } else {
        ...
        child: Column(
          children: <Widget>[
                  ...
                  children: <Widget>[
                      ...
                      child: v3.TextInputWidget(
                        decoration: v3.InputDecorationV3(
                          ...
                          suffixIcon: _args != null
                              ...
                              : GestureDetector(
                                  child: Padding(
                                    padding: EdgeInsets.only(left: 12.w, top: 10, bottom: 10),
                    ...
                    Row(
                      children: <Widget>[
                        ...
                        GestureDetector(
                          child: Text(I18n.of(context).getDic(i18n_full_dic_app, 'account')['observe']),
                          onTap: () {
                            setState(() {
                        ...
                        TapTooltip(
                          ...
                          message: I18n.of(context).getDic(i18n_full_dic_app, 'account')['observe.brief'],
                        ),
                      ],
                    )
                  ],
            ...
            Container(
              ...
              child: Button(submitting: _submitting, title: dic['contact.save'], onPressed: () => _onSave()),
              // child: RoundedButton(
