...
class _CreateRecoveryPage extends State<CreateRecoveryPage> {
  ...
  Future<void> _handleFriendsSelect() async {
    var res = await Navigator.of(context).pushNamed(FriendListPage.route, arguments: _friends);
    if (res != null) {
      setState(() {
  ...
  void _setDelay(String v, {bool custom = false}) {
    ...
    try {
      ...
      if (value == 0) {
        setState(() {
          _delayError = I18n.of(context).getDic(i18n_full_dic_app, 'profile')['input.invalid'];
        });
      } else {
        setState(() {
          _delay = value;
  ...
  void _onValidateSubmit() {
    ...
    String deposit = (_configDepositBase + _friends.length * _friendDepositFactor).toString();
      ...
      showCupertinoDialog(
        ...
        builder: (BuildContext context) {
          ...
          return CupertinoAlertDialog(
            title: Text('${dic['recovery.delay']} $_delay ${dic['recovery.day']}'),
  ...
  Future<void> _onSubmit() async {
    ...
    double deposit = _configDepositBase + _friends.length * _friendDepositFactor;

    final params =
        TxConfirmParams(txTitle: dic['recovery.create'], module: 'recovery', call: 'createRecovery', txDisplay: {
      'friends': friends,
      'threshold': _threshold.toInt(),
      'delay': '$_delay ${dic['recovery.day']}',
      'deposit': '${Fmt.doubleFormat(deposit)} ${widget.service.plugin.networkState.tokenSymbol[0]}'
    }, params: [
      friends,
      _threshold.toInt(),
      delayBlocks
...
''';
    ...
    double deposit = _configDepositBase + _friends.length * _friendDepositFactor;
    return Scaffold(
      appBar: AppBar(title: Text(dic['recovery.create']), centerTitle: true, leading: BackBtn()),
      body: SafeArea(
        child: Container(
                              ...
                              children: <Widget>[
                                CupertinoTextField(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    border: Border.all(width: 0.5, color: _delayFocusNode.hasFocus ? primary : grey),
                                  ),
                                  ...
                                  suffix: Container(
                                    ...
                                    child: Text(
                                      dic['recovery.day'],
                                      style: TextStyle(color: _delayFocusNode.hasFocus ? primary : grey),
                                    ),
                                  ),
                                ),
                                Visibility(
                                    ...
                                    child: Text(
                                      _delayError ?? "",
                                      style: TextStyle(color: Colors.red, fontSize: UI.getTextSize(12, context)),
                                    ))
                              ],
                            ),
                          )
