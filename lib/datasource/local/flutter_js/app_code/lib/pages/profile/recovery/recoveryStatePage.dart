...
class _RecoveryStatePage extends State<RecoveryStatePage> {
  ...
  Future<void> _fetchData() async {
    ...
    if (txs.length > 0) {
      ...
      final pubKeyAddressMap = await widget.service.plugin.sdk.api.account.encodeAddress(pubKeys);
      // set pubKeyAddressMap so we can parse the pubKey from subscan tx.
      widget.service.store.account
          .setPubKeyAddressMap(Map<String, Map>.from({'${widget.service.plugin.basic.ss58}': pubKeyAddressMap}));
      final addresses = pubKeys.map((e) => pubKeyAddressMap[e] as String).toList();
      ...
      List status = await Future.wait([
        ...
        widget.service.plugin.sdk.api.recovery.queryRecoveryProxies([widget.service.keyring.current.address]),
      ]);
  ...
  Future<void> _onAction(RecoveryInfo info, String action) async {
    ...
    final args = TxConfirmParams(
        ...
        module: 'recovery',
        call: action == _actionClaimRecovery ? 'claimRecovery' : 'cancelRecovered',
  ...
  Widget build(BuildContext context) {
    ...
    _txs.asMap().forEach((i, v) {
      bool isRecovered = _proxyStatus.indexOf(_recoverableInfoList[i].address) >= 0;
      activeList.add([
        v,
    ...
    return Scaffold(
      appBar: AppBar(title: Text(dic['recovery.init']), centerTitle: true, leading: BackBtn()),
      body: SafeArea(
        child: Container(
            ...
            children: [
                  ...
                  child: ListView(
                    ...
                    children: _txs.length > 0
                        ? activeList.map((e) {
                            ...
                            final bool canClaim = List.of(e[1]['friends']).length >= info.threshold &&
                                (createdBlock + info.delayPeriod) < _currentBlock;
                            ...
                            return ActiveRecovery(
                              ...
                              action: CupertinoActionSheetAction(
                                child: Text(canCancel ? dic['recovery.cancel'] : dic['recovery.claim']),
                                onPressed: canClaim
                                    ? () {
                                        Navigator.of(context).pop();
                                        if (canCancel) {
                                          _onAction(info, _actionCancelRecovery);
                                        } else {
              ...
              Padding(
                ...
                child: RoundedButton(
                  ...
                  onPressed: () => Navigator.of(context).pushNamed(InitiateRecoveryPage.route),
                ),
              )
            ],
          ),
