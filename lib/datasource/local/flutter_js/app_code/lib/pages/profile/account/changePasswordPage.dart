...
import 'package:seeds/datasource/local/flutter_js/app_code/lib/utils/format.dart';
...
class _ChangePassword extends State<ChangePasswordPage> {
  ...
  Future<void> _doChangePass(String passOld) async {
    ...
    await widget.service.plugin.sdk.api.keyring.changePassword(widget.service.keyring, passOld, passNew);

    final pubKey = widget.service.keyring.current.pubKey;
    if (false && _enableBiometric) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(dic['pass.change']), centerTitle: true, leading: BackBtn()),
      body: SafeArea(
        child: Column(
          children: <Widget>[
            Expanded(
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        v3.TextInputWidget(
                          ...
                          validator: (v) {
                            return AppFmt.checkPassword(v.trim()) ? null : accDic['create.password.error'];
                          },
                          obscureText: true,
                        ),
                        Container(
