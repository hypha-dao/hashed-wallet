...
class ExportAccountPage extends StatelessWidget {
  ...
  Future<void> _onExport(BuildContext context) async {
    ...
    if (password != null) {
      final seed = await service.plugin.sdk.api.keyring.getDecryptedSeed(service.keyring, password);
                  ...
                  children: [
                    SettingsPageListItem(
                      ...
                      onTap: () {
                        ...
                        data.type = 'keystore';
                        Navigator.of(context).pushNamed(ExportResultPage.route, arguments: data);
                      },
                    ),
                    FutureBuilder(
                      future: service.keyring.store.checkSeedExist(KeyType.mnemonic, service.keyring.current.pubKey),
                    ...
                    FutureBuilder(
                      future: service.keyring.store.checkSeedExist(KeyType.rawSeed, service.keyring.current.pubKey),
                      builder: (BuildContext context, AsyncSnapshot<bool> snapshot) {
                        if (snapshot.hasData && snapshot.data == true) {
                          return Column(
                            children: [
