...
class _VouchRecoveryPage extends State<VouchRecoveryPage> {
  ...
  Future<void> _onValidateSubmit() async {
    ...
    var info = await widget.service.account.queryRecoverable(_addressOldCtrl.text.trim());
    if (info == null) {
      address = addressOld;
      errorMsg = dic['recovery.not.recoverable'];
    } else {
      /// check if there is an active recovery for new account
      info = (await widget.service.plugin.sdk.api.recovery.queryActiveRecoveryAttempts(
  ...
  Future<void> _onSubmit(String addressOld, String addressNew) async {
    final dic = I18n.of(context).getDic(i18n_full_dic_app, 'profile');
    final args = TxConfirmParams(txTitle: dic['recovery.help'], module: 'recovery', call: 'vouchRecovery', txDisplay: {
      'lost': addressOld,
      'rescuer': addressNew,
    }, params: [
      addressOld,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(dic['recovery.help']), centerTitle: true, leading: BackBtn()),
      body: SafeArea(
        child: Container(
                ...
                child: ListView(
                  ...
                  children: [
                    TextFormField(
                          ...
                          onTap: () async {
                            var to = await Navigator.of(context).pushNamed(
                              ...
                              arguments: AccountListPageParams(
                                  title: I18n.of(context).getDic(i18n_full_dic_app, 'profile')['contact'],
                                  list: widget.service.keyring.allWithContacts),
                            );
                            if (to != null) {
                              setState(() {
                                _addressOldCtrl.text = (to as KeyPairData).address;
                              });
                            }
                          },
                        ),
                      ...
                      validator: (v) {
                        return Fmt.isAddress(v.trim()) ? null : dic['address.error'];
                      },
                    ),
                    TextFormField(
                      decoration: InputDecoration(
                        ...
                        suffix: GestureDetector(
                          ...
                          onTap: () async {
                            ...
                            if (to != null) {
                              setState(() {
                                _addressNewCtrl.text = (to as KeyPairData).address;
                              });
                            }
                          },
                        ),
