// @dart=2.9
import 'package:seeds/datasource/local/flutter_js/app_code/lib/common/components/jumpToLink.dart';
...
class NetworkSelectPage extends StatefulWidget {
  NetworkSelectPage(this.service, this.plugins, this.disabledPlugins, this.changeNetwork);
...
class _NetworkSelectPageState extends State<NetworkSelectPage> {
  ...
  Future<void> _onSelect(KeyPairData i) async {
    bool isCurrentNetwork = _selectedNetwork.basic.name == widget.service.plugin.basic.name;
    if (i.address != widget.service.keyring.current.address || !isCurrentNetwork) {
      /// set current account
  ...
  Future<void> _onCreateAccount(int step) async {
    ...
    Navigator.of(context).pushNamed(step == 0 ? CreateAccountPage.route : SelectImportTypePage.route);
  }

  List<Widget> _buildAccountList() {
    final dic = I18n.of(context).getDic(i18n_full_dic_app, 'account');
    final name = _selectedNetwork?.basic?.name ?? _pluginDisabledSelected?.name ?? '';
    final List<Widget> res = [
      Text(
        name.toUpperCase(),
        style: Theme.of(context).textTheme.headline3,
      ),
      Visibility(visible: plugin_from_community.indexOf(name) > -1, child: _CommunityPluginNote(name, false)),
      Padding(
    ...
    if (_selectedNetwork != null) {
      res.addAll(accounts.map((i) {
        final bool isCurrentNetwork = _selectedNetwork.basic.name == widget.service.plugin.basic.name;
        final addressMap = widget.service.keyring.store.pubKeyAddressMap[_selectedNetwork.basic.ss58.toString()];
        final address = addressMap != null ? addressMap[i.pubKey] : widget.service.keyring.current.address;
        final String accIndex = isCurrentNetwork && i.indexInfo != null && i.indexInfo['accountIndex'] != null
            ? '${i.indexInfo['accountIndex']}\n'
            : '';
        final isCurrent = isCurrentNetwork && i.address == widget.service.keyring.current.address;
        return Column(
          children: [
            Container(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          title: Text(I18n.of(context).getDic(i18n_full_dic_app, 'profile')['setting.network']),
          centerTitle: true,
          leading: BackBtn(),
          elevation: 2),
      body: Row(
        children: <Widget>[
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.only(topRight: Radius.circular(24.w)),
                      boxShadow: [
                        BoxShadow(
                            color: Color(0x33000000), blurRadius: 3.0, spreadRadius: 1.0, offset: Offset(2.0, 2.0))
                      ],
                    ),
                  ),
                  Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      width: 34.w,
                                      decoration: BoxDecoration(
                                          border: Border.symmetric(horizontal: BorderSide(color: Color(0xFFCCCAC6)))),
                                    )
                                  ],
                                ),
                              ),
                              ...widget.plugins.map((e) {
                                final isCurrent = e.basic.name == _selectedNetwork?.basic?.name;
                                return isCurrent
          ...
          Expanded(
            child: ListView(
              ...
              children: _pluginDisabledSelected == null ? _buildAccountList() : _buildPluginDisabled(),
            ),
          )
        ],
      ),
              ...
              child: Container(
                  ...
                  decoration: BoxDecoration(
                      image: DecorationImage(
                          image: AssetImage("assets/images/selectWallet_unselect.png"), fit: BoxFit.fill)))),
        ));
  }
}
    ...
    return Stack(
      ...
      children: [
        ...
        Container(
            ...
            decoration: BoxDecoration(
                image: DecorationImage(image: AssetImage("assets/images/selectWallet_select.png"), fit: BoxFit.fill)))
      ],
    );
  }
}
        ...
        children: [
          Row(
            children: [
              Expanded(
                  child: Text(
                dic['plugin.note'] + pluginName.toUpperCase() + dic['plugin.team'],
                style: TextStyle(fontSize: UI.getTextSize(12, context)),
              )),
              SvgPicture.asset('assets/images/public/github_logo.svg', width: 16),
              JumpToLink(
