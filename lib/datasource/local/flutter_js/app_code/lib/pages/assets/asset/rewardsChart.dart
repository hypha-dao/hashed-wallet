...
class RewardsChart extends StatelessWidget {
  ...
  final bool isRose;
  static int xBase = 10;
  RewardsChart(this.seriesList, this.maxX, this.maxY, this.minX, this.minY, this.width, this.isRose);
  ...
  factory RewardsChart.withData(List<TimeSeriesAmount> data, double width) {
    ...
    data.forEach((element) {
      ...
      if (maxX == null || element.time.millisecondsSinceEpoch > maxX.millisecondsSinceEpoch) {
        maxX = element.time;
      }
      if (minX == null || element.time.millisecondsSinceEpoch < minX.millisecondsSinceEpoch) {
        minX = element.time;
      }
    });
    ...
    return RewardsChart(flSpotDatas, maxX, maxY, minX, minY, width, data[data.length - 1].amount - data[0].amount < 0);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      ...
      children: [
        Padding(
            padding: EdgeInsets.only(right: 2.2 / 91 * chartWidth, bottom: 1 / 91.0 * chartWidth),
            child: ClipRRect(
        ...
        Container(
          ...
          decoration: BoxDecoration(
              image: DecorationImage(image: AssetImage("assets/images/rewards_bg.png"), fit: BoxFit.contain)),
        )
      ],
    );
  }
  ...
  List<Color> chartBelowBarColors() {
    if (isRose) {
      return [Color(0xFFBFFFD6).withOpacity(1), Color(0xFFBFFFD6).withOpacity(0)];
    } else {
      return [Color(0xFFCD4337).withOpacity(1), Color(0xFFCD4337).withOpacity(0)];
    }
  }
}
