...
class NodeSelectPage extends StatefulWidget {
  NodeSelectPage(this.service, this.plugins, this.changeNetwork, {Key key}) : super(key: key);
...
class _NodeSelectPageState extends State<NodeSelectPage> {
  ...
  void initState() {
    super.initState();
    expansionIndex = widget.plugins.indexWhere((element) => element.basic.name == widget.service.plugin.basic.name);
  }
            ...
            child: Stack(
              children: [
                Align(
                  ...
                  child: Text(I18n.of(context).getDic(i18n_full_dic_app, 'assets')["v3.changeNetwork"],
              ...
              child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return NodeSelectItem(
                        widget.service, widget.plugins[index], index == expansionIndex, widget.changeNetwork,
                        (itemIndex) {
...
class NodeSelectItem extends StatelessWidget {
  NodeSelectItem(this.service, this.plugin, this.isExpansion, this.changeNetwork, this.networkOnTap, this.index,
      {Key key})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          PluginItemWidget(
            leading: SizedBox(child: plugin.basic.icon, height: 42.w, width: 42.w),
            spacing: 11.w,
            bgColor: Theme.of(context).cardColor,
            title: Text("${plugin.basic.name[0].toUpperCase()}${plugin.basic.name.substring(1)}",
                style: Theme.of(context)
                    .textTheme
                    .headline4
                    .copyWith(fontWeight: FontWeight.w600, fontFamily: UI.getFontFamily('SF_Pro', context))),
                      ...
                      children: [
                        ...plugin.nodeList.map((e) {
                          return NodeItemWidget(
                            title: Text(e.name,
                                style: Theme.of(context).textTheme.headline4.copyWith(color: Color(0xFF040404))),
                            subtitle: Text(e.endpoint,
                                style: Theme.of(context)
                                    .textTheme
                                    .headline6
                                    .copyWith(color: Color(0xFF7E7D7B), fontWeight: FontWeight.w300)),
                            bgColor: Theme.of(context).cardColor,
                            isSelect: service.plugin.sdk.api.connectedNode?.endpoint == e.endpoint,
                            onTap: () {
                              Navigator.of(context).pop();
                              if (service.plugin.sdk.api.connectedNode?.endpoint == e.endpoint) return;
...
class PluginItemWidget extends StatefulWidget {
  PluginItemWidget({Key key, this.leading, this.title, this.spacing, this.onTap, this.isSelect, this.bgColor})
      : super(key: key);
  Widget leading;
  Widget title;
  double spacing;
...
class NodeItemWidget extends StatefulWidget {
  NodeItemWidget({Key key, this.title, this.subtitle, this.onTap, this.bgColor, this.isSelect}) : super(key: key);
  Widget title;
  Widget subtitle;
  Color bgColor;
  Function() onTap;
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
            color: widget.bgColor,
            image: isTapDown
                ? DecorationImage(image: AssetImage("assets/images/nodeItem_bg_onTapDown.png"), fit: BoxFit.fill)
                : null),
        padding: EdgeInsets.symmetric(horizontal: 16.w),
        child: Column(
          mainAxisSize: MainAxisSize.min,
