...
class LocksDetailPageState extends State<LocksDetailPage> {
  ...
  Future<void> _queryDemocracyLocks() async {
    final res = await widget.service.plugin.sdk.webView.evalJavascript('api.derive.chain.bestNumber()');
    bestNumber = int.parse(res.toString());
    final List locks =
        await widget.service.plugin.sdk.api.gov.getDemocracyLocks(widget.service.keyring.current.address);
  ...
  void _onUnlock(List<String> ids) async {
    ...
    final txs = ids.map((e) => 'api.tx.democracy.removeVote(${BigInt.parse(e)})').toList();
    txs.add('api.tx.democracy.unlock("${widget.service.keyring.current.address}")');
  ...
  Future<void> _updateVestingInfo() async {
    final res = await Future.wait([
      WalletApi.fetchBlocksFromSn(
          widget.service.plugin.basic.name == para_chain_name_karura ? relay_chain_name_ksm : relay_chain_name_dot),
      widget.service.plugin.sdk.webView
          .evalJavascript('api.query.vesting.vestingSchedules("${widget.service.keyring.current.address}")')
    ]);
    if (res[0] != null && res[1] != null) {
      ...
      List.from(res[1]).forEach((e) {
        ...
        final unlockingPeriod = endBlock - blockNowOrStart > BigInt.zero
            ? (endBlock - blockNowOrStart) ~/ periodBlocks + (startBlock > blockNow ? BigInt.zero : BigInt.one)
            : BigInt.zero;
      ...
      final locks = widget.service.plugin.balances.native.lockedBreakdown.toList();
  ...
  Future<void> _claimVest(String claimableAmount, int decimals, String symbol) async {
    final dic = I18n.of(context).getDic(i18n_full_dic_app, 'assets');
  ...
  Widget build(BuildContext context) {
    ...
    final locks = l.where((element) => element.use.contains('ormlvest')).toList();
    locks.addAll(l.where((element) => element.use.contains('democrac')).toList());
    l.retainWhere((e) => !e.use.contains('ormlvest') && !e.use.contains('democrac'));
    if (l.length > 0) {
      locks.add(BalanceBreakdownData.fromJson({"amount": 0, "use": ""}));
    }

    final hasClaim = _claimable != null && _claimable > BigInt.zero && !_submitting;
    final claimableAmount = Fmt.priceFloorBigInt(_claimable, decimals);
    return Scaffold(
      appBar: AppBar(title: Text('${dic['unlock']} ($symbol)'), centerTitle: true, leading: BackBtn()),
      body: SafeArea(
        child: Column(
          children: [
            Expanded(
                        ...
                        children: locks.map((e) {
                          ...
                          if (e.use.contains('democrac') && _locks.length > 0) {
                            for (int index = 0; index < _locks.length; index++) {
                              ...
                              if (unlockAt != "0") {
                                BigInt endLeft;
                                try {
                                  endLeft = BigInt.parse("${unlockAt.toString()}") - BigInt.from(bestNumber);
                                } catch (e) {
                                  endLeft = BigInt.parse("0x${unlockAt.toString()}") - BigInt.from(bestNumber);
                                }
                                if (endLeft.toInt() <= 0) {
                            ...
                            Democracchild = Column(
                              children: [
                                InfoItemRow(dic['lock.democrac.total'], "${maxLockAmount + maxUnlockAmount}"),
                                InfoItemRow(dic['lock.vest.unlocking'], "$maxLockAmount"),
                          ...
                          if (e.use.contains('ormlvest')) {
                            return buildItem(
                                ...
                                child: Column(
                                  children: [
                                    _originalLocked != null
                                        ...
                                        : Container(),
                                    InfoItemRow(
                                        dic['lock.vest'], '${Fmt.priceFloorBigInt(amt, decimals, lengthMax: 4)}'),
                                    InfoItemRow(dic['lock.vest.unlocking'],
                                        Fmt.priceFloorBigInt(_unlocking, decimals, lengthMax: 4)),
                                    _originalLocked != null
                                    ...
                                    hasClaim
                                        ? InfoItemRow(
                                            dic['lock.vest.claimable'],
                                            claimableAmount,
                                            labelStyle: Theme.of(context).textTheme.headline5.copyWith(fontSize: 18),
                                            contentStyle: Theme.of(context).textTheme.headline5.copyWith(
                                                fontSize: UI.getTextSize(18, context),
                                                color: Color(0xFFE46B41),
                                                fontWeight: FontWeight.w600),
                                ...
                                hasClaim: hasClaim,
                                onRedeem: () => _claimVest(claimableAmount, decimals, symbol));
                                ...
                                child: Column(
                                  children: [
                                    ...l
                                        .map((e) => InfoItemRow(
                                            dic['lock.${e.use.trim()}'],
                                            Fmt.priceFloorBigInt(BigInt.parse(e.amount.toString()), decimals,
                                                lengthMax: 4)))
                                        .toList()
                                  ],
                                ),
  ...
  Widget buildItem(
      {@required String title, @required Widget child, @required bool hasClaim, @required Function onRedeem}) {
    return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(padding: EdgeInsets.only(top: 2), child: InnerShadowBGCar(child: child)),
            hasClaim
                ? Container(
                        ...
                        GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                image:
                                    DecorationImage(image: AssetImage("assets/images/icon_bg_2.png"), fit: BoxFit.fill),
                              ),
                              alignment: Alignment.center,
                              child: Text(
                                I18n.of(context).getDic(i18n_full_dic_app, 'assets')['lock.vest.claim'],
                                style: TextStyle(
