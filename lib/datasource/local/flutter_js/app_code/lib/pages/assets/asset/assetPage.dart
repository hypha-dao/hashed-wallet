...
import 'package:seeds/datasource/local/flutter_js/app_code/lib/pages/assets/asset/rewardsChart.dart';
...
import 'package:seeds/datasource/local/flutter_js/app_code/lib/utils/ShowCustomAlterWidget.dart';
...
class _AssetPageState extends State<AssetPage> {
        ...
        actions: <Widget>[
          CupertinoActionSheetAction(
            child: Row(
              ...
              children: [
                Text(I18n.of(context).getDic(i18n_full_dic_app, 'assets')['address.subscan']),
                Padding(
            ...
            onPressed: () {
              ...
              final snLink = 'https://$networkName.subscan.io/account/${widget.service.keyring.current.address}';
              UI.launchURL(snLink);
  ...
  void initState() {
    ...
    WalletApi.getMarketPriceList((widget.service.plugin.networkState.tokenSymbol ?? [''])[0], 7).then((value) {
      if (mounted) {
        setState(() {
          if (value['data'] != null) {
  ...
  List<TimeSeriesAmount> getTimeSeriesAmounts(List<dynamic> marketPriceList) {
    ...
    for (int i = 0; i < marketPriceList.length; i++) {
      datas.add(TimeSeriesAmount(DateTime.now().add(Duration(days: -1 * i)), i * 1.0));
    }
    return datas;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          symbol,
          style: TextStyle(fontSize: UI.getTextSize(20, context), color: Colors.black87),
        ),
        centerTitle: true,
      ...
      body: Observer(
        builder: (_) {
          ...
          final txs = widget.service.plugin
              .getNativeTokenTransfers(address: widget.service.keyring.current.address, transferType: _tab);
          return Column(
            children: <Widget>[
              BalanceCard(
                balancesInfo,
                symbol: symbol,
                decimals: decimals,
                marketPrices: (widget.service.store.assets.marketPrices[symbol] ?? 0) *
                    ...
                    children: <Widget>[
                            ...
                            onPressed: () {
                              if (widget.service.plugin.basic.name == para_chain_name_karura ||
                                ...
                                Navigator.of(context)
                                    .pushNamed('/assets/token/transfer', arguments: {'tokenNameId': symbol});
                                return;
                              }
                      ...
                      Expanded(
                        child: Container(
                          padding: EdgeInsets.symmetric(horizontal: 3.w),
                          child: CardButton(
                            icon: Image.asset("assets/images/qr.png", width: 37),
                            text: dic['receive'],
                            onPressed: () {
                      ...
                      Fmt.balanceInt((balancesInfo?.lockedBalance ?? 0).toString()) > BigInt.one
                          ? Expanded(
                              child: Container(
                                padding: EdgeInsets.symmetric(horizontal: 3.w),
                                child: CardButton(
                                  icon: Image.asset("assets/images/unlock.png", width: 37),
                                  text: dic['unlock'],
                                  onPressed: Fmt.balanceInt((balancesInfo?.lockedBalance ?? 0).toString()) > BigInt.one
                                      ? () {
                                          Navigator.pushNamed(context, LocksDetailPage.route);
                                        }
                                      : null,
                                ),
                              ),
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              image: DecorationImage(image: AssetImage("assets/images/bg_tag.png"), fit: BoxFit.fill),
                            ),
                            child: Center(
                              child: Text(
                                  dic[_tab == 0
                                  ...
                                  style: Theme.of(context).textTheme.headline5.copyWith(
                                      color: Theme.of(context).toggleableActiveColor, fontWeight: FontWeight.w600)),
                            ),
                          ),
                          GestureDetector(
                              onTap: () {
                                showCupertinoModalPopup(
                                    ...
                                    builder: (context) {
                                      return ShowCustomAlterWidget(
                                          ...
                                          cancel: I18n.of(context).getDic(i18n_full_dic_ui, 'common')['cancel'],
                                          options: [dic['all'], dic['in'], dic['out']]);
                                    });
                              },
                              child: v3.IconButton(
...
class BalanceCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (marketPrices != null && balancesInfo != null) {
      tokenPrice = Fmt.priceFloor(marketPrices * Fmt.bigIntToDouble(balance, decimals));
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          colors: bgColors ?? [Theme.of(context).primaryColor, Theme.of(context).hoverColor],
                      ...
                      Visibility(
                        ...
                        child: Text(
                          ...
                          style: Theme.of(context)
                              .textTheme
                              .headline6
                              .copyWith(color: titleColor, letterSpacing: -0.8, fontWeight: FontWeight.w600),
                        ),
                      ),
                    ],
                  ),
                  ...
                  children: [
                    priceItemBuild(
                        ...
                        Fmt.priceFloorBigInt(
                          Fmt.balanceInt((balancesInfo?.availableBalance ?? 0).toString()),
                          decimals,
                          lengthMax: 4,
                        ),
                        titleColor,
                    ...
                    priceItemBuild(
                        ...
                        dic['reserved'],
                        Fmt.priceFloorBigInt(
                          Fmt.balanceInt((balancesInfo?.reservedBalance ?? 0).toString()),
                          decimals,
                          lengthMax: 4,
                        ),
                        titleColor,
                ...
                child: marketPriceList != null
                    ? Container(
                        ...
                        child: RewardsChart.withData(
                            getTimeSeriesAmounts(marketPriceList), MediaQuery.of(context).size.width / 4))
  ...
  List<TimeSeriesAmount> getTimeSeriesAmounts(List<dynamic> marketPriceList) {
    List<TimeSeriesAmount> datas = [];
    for (int i = 0; i < marketPriceList.length; i++) {
      datas.add(TimeSeriesAmount(DateTime.now().add(Duration(days: -1 * i)), marketPriceList[i] * 1.0));
    }
    return datas;
  }

  Widget priceItemBuild(Widget icon, String title, String price, Color color, BuildContext context) {
    return Padding(
        padding: EdgeInsets.symmetric(vertical: 3),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Container(height: 16.w, width: 16.w, margin: EdgeInsets.only(right: 8.w), child: icon),
            Text(
              title,
              style: TextStyle(
                  color: color,
...
class TransferListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final title = Fmt.address(address) ?? data.extrinsicIndex ?? Fmt.address(data.hash);
    ...
    return ListTile(
      ...
      subtitle: Text(
        Fmt.dateTime(DateTime.fromMillisecondsSinceEpoch(data.blockTimestamp * 1000)),
        style: Theme.of(context).textTheme.headline6.copyWith(fontWeight: FontWeight.w300, color: Color(0xBF565554)),
      ),
      trailing: Container(
        width: 110,
        child: Row(
            ...
            Expanded(
              child: Text(
                ...
                style: Theme.of(context)
                    .textTheme
                    .headline5
                    .copyWith(color: Theme.of(context).toggleableActiveColor, fontWeight: FontWeight.w600),
