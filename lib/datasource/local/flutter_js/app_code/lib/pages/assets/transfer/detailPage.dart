...
class TransferDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final Map<String, String> dic = I18n.of(context).getDic(i18n_full_dic_app, 'assets');
    ...
    final String txType = tx.from == service.keyring.current.address ? dic['transfer'] : dic['receive'];
    ...
    return TxDetail(
      ...
      infoItems: <TxDetailInfoItem>[
        TxDetailInfoItem(
          label: dic['amount'],
          content: Text('${txType == dic['transfer'] ? "-" : ""}$amount $symbol',
              style: Theme.of(context).textTheme.headline1),
        ),
        TxDetailInfoItem(
          label: dic['from'],
          content: Text(Fmt.address(tx.from), style: Theme.of(context).textTheme.headline4),
          copyText: tx.from,
        ),
        TxDetailInfoItem(
          label: dic['to'],
          content: Text(Fmt.address(tx.to), style: Theme.of(context).textTheme.headline4),
          copyText: tx.to,
        )
      ],
    );
