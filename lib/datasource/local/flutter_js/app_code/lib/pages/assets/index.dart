...
import 'package:seeds/datasource/local/flutter_js/app_code/lib/common/components/CustomRefreshIndicator.dart';
...
import 'package:seeds/datasource/local/flutter_js/app_code/lib/pages/assets/nodeSelectPage.dart';
...
import 'package:seeds/datasource/local/flutter_js/app_code/lib/pages/public/AdBanner.dart';
import 'package:seeds/datasource/local/flutter_js/app_code/lib/service/index.dart';
import 'package:seeds/datasource/local/flutter_js/app_code/lib/utils/InstrumentWidget.dart';
...
final assetsType = ["All", "Native", "ERC-20", "Cross-chain", "LP Tokens", "Taiga token"];
...
class _AssetsState extends State<AssetsPage> {
  final GlobalKey<CustomRefreshIndicatorState> _refreshKey = new GlobalKey<CustomRefreshIndicatorState>();
  ...
  Future<void> _updateMarketPrices() async {
    widget.service.assets.fetchMarketPrices(widget.service.plugin.networkState.tokenSymbol);

    final duration = widget.service.store.assets.marketPrices.keys.length > 0 ? 60 : 6;
  ...
  Future<void> _handleScan() async {
    ...
    if (data != null) {
      if (data.type == QRCodeResultType.rawData && data.rawData.substring(0, 3) == 'wc:') {
      ...
      if (data.type == QRCodeResultType.address) {
        if (widget.service.plugin.basic.name == para_chain_name_karura ||
          ...
          Navigator.of(context)
              .pushNamed('/assets/token/transfer', arguments: {'tokenNameId': symbol, 'address': data.address.address});
          return;
        }
      ...
      try {
        final qrData =
            await widget.service.plugin.sdk.api.uos.parseQrCode(widget.service.keyring, data.rawData.toString().trim());
        Navigator.of(context).pop();

        final networkIndex = widget.plugins.indexWhere((e) => e.basic.genesisHash == qrData.genesisHash);
        ...
        if (qrData.genesisHash != null && networkIndex < 0) {
          ...
          final sender = widget.service.keyring.keyPairs.firstWhere((e) => e.pubKey == qrData.signer);
          ...
          final confirmed = await showCupertinoDialog(
            ...
            builder: (_) {
              return CupertinoAlertDialog(
                title: Text(dic['uos.title']),
                content: Column(crossAxisAlignment: CrossAxisAlignment.start, children: confirmMsg),
                actions: <Widget>[
                  CupertinoButton(
          ...
          if (confirmed) {
            if (needSwitchAccount) {
              ...
              widget.service.store.assets.loadCache(sender, widget.service.plugin.basic.name);
            }

            final password = await widget.service.account.getPassword(context, widget.service.keyring.current);
            if (password != null) {
  ...
  Future<void> _signAsync(String password) async {
    ...
    try {
      ...
      final signed = await widget.service.plugin.sdk.api.uos.signAsync(widget.service.plugin.basic.name, password);
      print('signed: $signed');
      ...
      showCupertinoDialog(
        ...
        builder: (_) {
          return CupertinoAlertDialog(
            ...
            actions: <Widget>[
              CupertinoButton(
                child: Text(I18n.of(context).getDic(i18n_full_dic_ui, 'account')['ok']),
                onPressed: () => Navigator.of(context).pop(),
              ),
            ],
          );
  ...
  List<Color> _gradienColors() {
    switch (widget.service.plugin.basic.name) {
      ...
      case para_chain_name_bifrost:
        return [Color(0xFF5AAFE1), Color(0xFF596ED2), Color(0xFFB358BD), Color(0xFFFFAE5E)];
      default:
  ...
  List<InstrumentData> _instrumentDatas() {
    ...
    final available = marketPrice *
        Fmt.bigIntToDouble(
          Fmt.balanceInt((widget.service.plugin.balances.native?.availableBalance ?? 0).toString()),
          decimals,
        );
    ...
    InstrumentData totalBalance = InstrumentData(available + reserved + locked, [], title: title);
    totalBalance.items.add(
        InstrumentItemData(Color(0xFFCE623C), dic['reserved'], reserved, "assets/images/icon_instrument_orange.png"));
    totalBalance.items
        .add(InstrumentItemData(Color(0xFFFFC952), dic['locked'], locked, "assets/images/icon_instrument_yellow.png"));
    totalBalance.items.add(
        InstrumentItemData(Color(0xFF768FE1), dic['available'], available, "assets/images/icon_instrument_blue.png"));
  ...
  PreferredSizeWidget buildAppBar() {
    return AppBar(
      ...
      title: Row(
        ...
        children: [
          Container(
            child: AddressIcon(widget.service.keyring.current.address, svg: widget.service.keyring.current.icon),
                      ...
                      showModalBottomSheet(
                        ...
                        builder: (BuildContext context) {
                          return Container(
                            ...
                            child: NodeSelectPage(widget.service, widget.plugins, widget.switchNetwork),
                          );
                        },
                        context: context,
                      );
      ...
      leading: v3.IconButton(
        ...
        onPressed: widget.service.keyring.allAccounts.length > 0
            ? () async {
                final selected = (await Navigator.of(context).pushNamed(NetworkSelectPage.route)) as PolkawalletPlugin;
                setState(() {});
                if (selected != null && selected.basic.name != widget.service.plugin.basic.name) {
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Text(
                              I18n.of(context).getDic(i18n_full_dic_app, 'assets')['scan'],
                              style: Theme.of(context).textTheme.headline5,
                            ),
                          )
                        ],
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Padding(
                            padding: EdgeInsets.only(left: 5),
                            child: Text(
                              I18n.of(context).getDic(i18n_full_dic_app, 'assets')['QRCode'],
                              style: Theme.of(context).textTheme.headline5,
                            ),
                          )
                        ],
    ...
    return Observer(
      builder: (_) {
        ...
        if (defaultTokens.length > 0) {
          tokens.retainWhere((e) => e.symbol.contains('-') || defaultTokens.indexOf(e.tokenNameId) > -1);
        }
        ...
        String tokenPrice;
        if (widget.service.store.assets.marketPrices[symbol] != null && balancesInfo != null) {
          tokenPrice = Fmt.priceCeil(widget.service.store.assets.marketPrices[symbol] *
              (widget.service.store.settings.priceCurrency == "CNY" ? _rate : 1.0) *
                        ...
                        Padding(
                          ...
                          child: instrumentIndex == 0 ||
                                  widget.service.plugin
                                          .getAggregatedAssetsWidget(onSwitchBack: null, onSwitchHideBalance: null) ==
                                      null
                              ? InstrumentWidget(
                                  _instrumentDatas(),
                                  ...
                                  onSwitchHideBalance: () {
                                    widget.service.store.settings
                                        .setIsHideBalance(!widget.service.store.settings.isHideBalance);
                                  },
                                  enabled: widget.connectedNode != null,
                                  hideBalance: widget.service.store.settings.isHideBalance,
                              ...
                              : widget.service.plugin.getAggregatedAssetsWidget(
                                  ...
                                  priceCurrency: widget.service.store.settings.priceCurrency,
                                  rate: widget.service.store.settings.priceCurrency == "CNY" ? _rate : 1.0,
                                  hideBalance: widget.service.store.settings.isHideBalance),
                        ),
                        Container(
                            ...
                            ? Padding(
                                ...
                                child: Row(
                                  children: [
                                    Expanded(
                                        child: TextTag(
                                      I18n.of(context).getDic(i18n_full_dic_app, 'assets')['assets.warn'],
                        ...
                        child: Column(
                          children: [
                            Row(
                              children: [
                                BorderedTitle(
                                  title: I18n.of(context).getDic(i18n_full_dic_app, 'assets')['assets'],
                                ),
                                Visibility(
                                    visible: (widget.service.plugin.noneNativeTokensAll ?? []).length > 0,
                                    child: Expanded(
                                        child: Row(
                                      mainAxisAlignment: MainAxisAlignment.end,
                                      children: [
                                        v3.IconButton(
                                          onPressed: () => Navigator.of(context).pushNamed(ManageAssetsPage.route),
                                          icon: Icon(
                                            Icons.menu,
                                            color: Theme.of(context).disabledColor,
                                            size: 20,
                            ...
                            Visibility(
                                visible: widget.service.plugin.basic.name == plugin_name_karura ||
                                    widget.service.plugin.basic.name == plugin_name_acala,
                                child: Container(
                                    height: 30,
                                    width: double.infinity,
                                        ...
                                        itemBuilder: (context, index) {
                                          return CupertinoButton(
                                            ...
                                            child: Container(
                                              ...
                                              width: 65,
                                              decoration: BoxDecoration(
                                                borderRadius: _assetsTypeIndex == index
                                                    ? null
                                                    : BorderRadius.all(Radius.circular(6.0)),
                                                color: _assetsTypeIndex == index ? Colors.transparent : Colors.white,
                                                border: _assetsTypeIndex == index
                                                    ? null
                                                    : Border.all(
                                                        color: Color(0xFF979797),
                                                        width: 0.2,
                                                ...
                                                image: _assetsTypeIndex == index
                                                    ? DecorationImage(
                                                        image: AssetImage('assets/images/icon_select_btn.png'),
                                                        fit: BoxFit.fill,
                                                      )
                                                    : null,
                                                boxShadow: _assetsTypeIndex == index
                                                    ? []
                                                    : [
                                                        BoxShadow(
                                                          offset: Offset(1, 1),
                                              ...
                                              child: Center(
                                                child: Text(assetsType[index],
                                                    style: Theme.of(context).textTheme.button?.copyWith(
                                                        color: _assetsTypeIndex == index ? Colors.white : Colors.black,
                        ...
                        children: [
                          RoundedCard(
                            ...
                            child: Column(
                              children: [
                                Visibility(
                                    visible: _assetsTypeIndex == 0 || _assetsTypeIndex == 1,
                                    child: ListTile(
                                      leading: Container(
                                        width: 48.w,
                                        ...
                                        children: [
                                          Text(
                                              balancesInfo != null && balancesInfo.freeBalance != null
                                                  ? widget.service.store.settings.isHideBalance
                                                      ? "******"
                                                      : Fmt.priceFloorBigInt(Fmt.balanceTotal(balancesInfo), decimals,
                                                          lengthFixed: 4)
                                                  : '--.--',
                                          ...
                                          Text(
                                            ...
                                            style: Theme.of(context)
                                                .textTheme
                                                .headline6
                                                .copyWith(fontFamily: UI.getFontFamily('TitilliumWeb', context)),
                                          ),
                                        ],
                                      ),
                                      onTap: () {
                                ...
                                Visibility(
                                    ...
                                    child: Column(
                                      children: (tokens ?? []).map((TokenBalanceData i) {
                                        ...
                                        final price =
                                            i.price ?? widget.service.store.assets.marketPrices[i.symbol] ?? 0.0;
                                        return TokenItem(
                                          i,
                                          i.decimals,
                                          ...
                                          isHideBalance: widget.service.store.settings.isHideBalance,
                                          priceCurrency: widget.service.store.settings.priceCurrency,
                                        );
                                      }).toList(),
                                    )),
                                        ...
                                        Column(
                                          children: i.tokens
                                              .map((e) => TokenItem(
                                                    ...
                                                    icon: widget.service.plugin.tokenIcons[e.symbol],
                                                    isHideBalance: widget.service.store.settings.isHideBalance,
                                                    priceCurrency: widget.service.store.settings.priceCurrency,
                                                  ))
                                              .toList(),
...
class TokenItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final balanceTotal = Fmt.balanceInt(item.amount) + Fmt.balanceInt(item.reserved);
    return Column(
      children: [
        Divider(height: 1),
        ListTile(
          ...
          trailing: Column(
            ...
            children: [
              Text(
                isHideBalance ? "******" : Fmt.priceFloorBigInt(balanceTotal, decimals, lengthFixed: 4),
                style: Theme.of(context).textTheme.headline5.copyWith(
                    fontWeight: FontWeight.w600,
